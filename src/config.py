...
# لایه ۶ کشور را برای *هر IP یکتا* می‌پرسد، نه هر کانفیگ. اجرای واقعی ۱۷:۴۶
# نشان داد ۴۷۵۴ از ۶۶۰۳ کانفیگ کشورشان «نامعلوم» ماند و کاربر در ربات فقط
# US می‌دید: ۶۶۰۰ کوئری همزمان سهمیه‌ی هر سه API را می‌سوزاند. با اندازه‌ی
# دسته‌ی ۱۰۰ (سقف ip-api/batch) همان کار با ~۲۰ درخواست انجام می‌شود.
GEO_BATCH_SIZE      = _int_env("GEO_BATCH_SIZE", 100)
# سقف زمان لایه ۶ به ثانیه. احترام به X-Ttl یعنی ممکن است منتظر بمانیم؛
# این سقف تضمین می‌کند انتظار، job را نخورد. بعد از پایان بودجه، IP های
# باقی‌مانده «نامعلوم» می‌مانند (پاس می‌شوند) — همان رفتار قبلی.
GEO_BUDGET_SEC      = _int_env("GEO_BUDGET_SEC", 420)
...
# سقف ورودی لایه ۷ (کانفیگ‌های تست‌نشده از خروجی حذف نمی‌شوند و به پول ذخیره
# می‌روند). با سقف ۱۵۰۰، اجرای ۱۷:۴۶ فقط ۷۲۳ کانفیگ تأییدشده ساخت و
# ۵۱۰۳ کانفیگِ پاس‌کرده‌ی لایه ۶ هیچ‌وقت آزموده نشدند — پول کوچک، و پول کوچک
# یعنی تکرار در کانال. حالا سقف بالاتر است و ترمزِ واقعی زمان است
# (HTTP_TEST_BUDGET_SEC)، پس تعداد به سرعتِ رانر تطبیق می‌یابد نه به یک حدس.
MAX_HTTP_TEST       = _int_env("MAX_HTTP_TEST", 4000)

# بودجه‌ی زمانی لایه ۷ به ثانیه. سقف job روی گیت‌هاب ۵۵ دقیقه است و لایه‌های
# ۱ تا ۶ در اجرای واقعی ~۴۰۰ ثانیه بردند؛ ۲۴۰۰ ثانیه (۴۰ دقیقه) برای لایه ۷
# جا می‌گذارد و ~۱۰ دقیقه حاشیه برای commit و README باقی می‌ماند. کانفیگی که
# نوبتش بعد از پایان بودجه برسد **رد نمی‌شود**، به پول ذخیره می‌رود:
# «تست نشد» ≠ «رد شد». صفر = بی‌بودجه (فقط MAX_HTTP_TEST ترمز است).
HTTP_TEST_BUDGET_SEC = _int_env("HTTP_TEST_BUDGET_SEC", 2400)

# ─── چرخشِ نوبتِ لایه ۷ (شرح کامل: src/tester/ledger.py) ────
# شکایت کاربر: «چرا حس می‌کنم ربات دیگر دنبال کانفیگ نیست و همان‌های قبلی را
# می‌فرستد؟» علت این‌جا بود: ورودی لایه ۷ فقط با «تأییدشده‌ی ایران، بعد
# سریع‌ترین TLS» مرتب می‌شد و بودجه دُم صف را می‌بُرید، پس هر اجرا همان
# سریع‌ترین‌ها آزموده می‌شدند و بقیه هیچ‌وقت به تونل نمی‌رسیدند.
#
# سهم تضمینی «هرگز آزموده‌نشده‌ها» از بودجه‌ی لایه ۷. ۰.۵ یعنی نیمی از صف برای
# کشفِ تازه است. صفر = رفتار قبلی (فقط سریع‌ترین‌ها).
HTTP_FRESH_SHARE     = _float_env("HTTP_FRESH_SHARE", 0.5)
# اولِ صف این تعداد از «تأییدشده‌های اجرای قبل» می‌نشیند تا اگر بودجه خیلی
# زود تمام شد، کانال باز هم کانفیگِ *واقعاً تأییدشده* برای یک دوره داشته
# باشد. ۳ × PUBLISH_COUNT: سه دوره‌ی انتشار حاشیه.
HTTP_PROVEN_CORE     = _int_env("HTTP_PROVEN_CORE", 30)
# کانفیگی که لایه ۷ ردش کرد تا این تعداد اجرا دوباره آزموده نمی‌شود (با نیم
# ساعت فاصله‌ی اجراها، ۶ ≈ ۳ ساعت). خرابیِ گذرا فرصت دوباره می‌گیرد، ولی
# بودجه صرفِ چیزی که همین حالا رد شد نمی‌شود.
HTTP_RETRY_AFTER_RUNS = _int_env("HTTP_RETRY_AFTER_RUNS", 6)
# فراموشیِ دفتر: رکورد قدیمی‌تر از این تعداد اجرا پاک می‌شود و کانفیگ دوباره
# «هرگز آزموده‌نشده» حساب می‌آید (۴۸ ≈ ۲۴ ساعت). دنیا عوض می‌شود؛ حکمِ دیروز
# برای همیشه معتبر نیست.
HTTP_LOG_KEEP_RUNS   = _int_env("HTTP_LOG_KEEP_RUNS", 48)
# سقف سخت رکوردهای دفتر تا فایلِ commit‌شونده بی‌مرز رشد نکند.
HTTP_LOG_MAX         = _int_env("HTTP_LOG_MAX", 20000)
...
# ─── کانال‌های تلگرام ──────────────────────────────────────
# مسیر اصلی بی‌نیاز از حساب است: صفحه‌ی پیش‌نمایش عمومی `t.me/s/<channel>`.
# عددِ کنار هر نام، کانفیگ یکتای اندازه‌گیری‌شده در *یک* صفحه (۲۰ پیام آخر)
# است — با ورق زدن چند برابر می‌شود. از ۵۷ کانالِ آزموده این ۲۹ تا جواب
# دادند؛ بقیه یا پیش‌نمایش عمومی ندارند یا فقط پروکسی MTProto می‌گذارند.
TELEGRAM_CHANNELS = [
    "proxy_mtm",              # 143
    "prrofile_purple",        # 124
    "V2rayNG3",               # 110
...
# چند صفحه از هر کانال ورق زده شود (هر صفحه ۲۰ پیام). ۵ صفحه ≈ ۱۰۰ پیامِ
# آخر هر کانال؛ عددِ بزرگ‌تر یعنی درخواست بیشتر به t.me و کندتر شدن فاز
# جمع‌آوری، بدون آنکه کانفیگ تازه‌ای اضافه شود (کانال‌ها روزی چند پست دارند).
TELEGRAM_PAGES = _int_env("TELEGRAM_PAGES", 5)
...
# دفترِ نوبتِ لایه ۷: هش کانفیگ + شماره‌ی اجرا + نتیجه. لینک کامل در آن
# نیست، پس فایل کوچک و بی‌خطر است. *باید* commit شود — تنها حافظه‌ای است که
# می‌گوید کدام کانفیگ‌ها قبلاً از تونل آزموده شده‌اند، و بدون آن هر اجرا از
# صفر شروع می‌کند و همان سریع‌ترین‌ها را دوباره می‌آزماید. (برخلاف
# publish_state.json که وضعیتِ ربات است و در .gitignore می‌ماند: نویسنده و
# خواننده‌ی این دفتر همان workflow است، پس commit شدنش درست است.)
LAYER7_LOG_FILE    = os.getenv("LAYER7_LOG_FILE", "configs/layer7_log.txt")

# ─── حافظه‌ی ماندگارِ همین ماشین ───────────────────────────
# فایل‌های بالا محصولِ pipeline اند و در مخزن commit می‌شوند. فایل‌های زیر
# *وضعیت* اند: مخصوص همین ماشین، و هیچ‌وقت commit نمی‌شوند.
#
# چرا مسیرشان جدا شد: ربات روی Railway اجرا می‌شود و configs/ را آن‌جا از
# raw.githubusercontent می‌خواند، نه از git. پس فایل وضعیت نه commit می‌شود
# نه بازیابی، و فایل‌سیستم کانتینر با هر redeploy پاک می‌شود — حافظه‌ی چرخش
# صفر می‌شد و همان ۱۰ کانفیگ صدرجدول از نو پست می‌شدند. این دقیقاً شکایت
# کاربر بود: «کانفیگ‌هایی که قبلاً کار کرده‌اند نباید دوباره فرستاده شوند.»
#
# Railway وقتی volume وصل باشد RAILWAY_VOLUME_MOUNT_PATH را خودش ست می‌کند،
# پس فقط وصل کردن volume در داشبورد کافی است و چیزی در کد یا env عوض نمی‌شود.
# بدون volume مسیر همان configs/ قبلی می‌ماند تا داده‌ی محلی جابه‌جا نشود.
STATE_DIR = os.getenv(
    "STATE_DIR", os.getenv("RAILWAY_VOLUME_MOUNT_PATH", "") or CONFIGS_DIR
)
# وضعیت چرخش انتشار (کدام کانفیگ‌ها در کدام دوره پست شده‌اند).
PUBLISH_STATE_FILE = os.getenv(
    "PUBLISH_STATE_FILE", os.path.join(STATE_DIR, "publish_state.json")
)
...
BOT_STATE_FILE     = os.getenv(
    "BOT_STATE_FILE", os.path.join(STATE_DIR, "bot_state.json")
)
...
# کفِ انتظار: یک کانفیگ تا وقتی این تعداد چرخه نگذرد دوباره پست نمی‌شود.
PUBLISH_COOLDOWN    = _int_env("PUBLISH_COOLDOWN", 6)
# انتظارِ خودتنظیم بر اساس اندازه‌ی پول. شکایت صریح کاربر: «کانفیگ‌هایی که
# قبلاً کار کرده‌اند نباید دوباره و دوباره فرستاده شوند.» با انتظارِ ثابتِ ۶
# دوره (۳۰ دقیقه) و پول ۷۲۳ تایی، بهترین کانفیگ نیم‌ساعت بعد واجد شرط می‌شد
# و چون سرِ صفِ کیفیت بود همان لحظه انتخاب می‌شد؛ ۶۶۰ کانفیگ دیگر هیچ‌وقت
# پست نمی‌شدند. با این کلید، انتظار = اندازه‌ی پول ÷ سهمیه‌ی هر دوره، پس پول
# یک بار کامل چرخیده می‌شود و بعد تکرار شروع می‌شود. خاموش کردنش رفتار
# قدیمی (انتظار ثابت) را برمی‌گرداند.
PUBLISH_COOLDOWN_AUTO = _bool_env("PUBLISH_COOLDOWN_AUTO", True)
# سقف انتظار. بی‌سقف، پولِ ۱۰هزارتایی یعنی کانفیگ عملاً هیچ‌وقت برنمی‌گردد و
# حافظه‌ی posted بی‌مرز رشد می‌کند. ۸۶۴ دوره × ۵ دقیقه = ۳ روز.
PUBLISH_COOLDOWN_MAX = _int_env("PUBLISH_COOLDOWN_MAX", 864)
...
# پس بی‌سقف بودنش مخزن را باد می‌کند. ۱۵۰۰ یعنی ۱۵۰ دوره‌ی انتشار ذخیره —
# با ۵۰۰ قبلی، ذخیره در ~۴ ساعت ته می‌کشید و انتخاب مجبور به تکرار می‌شد.
POOL_MAX                = _int_env("POOL_MAX", 1500)
